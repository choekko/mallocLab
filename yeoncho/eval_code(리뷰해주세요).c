...
/*
    comment) size_t 대신 int도 4byte이기 때문에 무관하지만, 주소값이기 때문에 양수로 표현되는게 직관적인 것 같습니다.
*/
...
// comment) 이렇게 깔끔하게 정리할 수 있다는 것 배워갑니다!!!
...
int mm_init(void) {
    ...
    PUT(heap_listp, 0); // comment) 패딩 넣어주기 (for 최소 16바이트 크기의 블록 구성)
    PUT(heap_listp + (1 * WSIZE), PACK(DSIZE, 1)); //프롤로그 헤더
    PUT(heap_listp + (2 * WSIZE), PACK(DSIZE, 1)); //프롤로그 풋터
    PUT(heap_listp + (3 * WSIZE), PACK(0, 1)); //에필로그 풋터
    heap_listp += (2 * WSIZE); // comment) 프롤로그 헤더, 풋터 사이로 옮겨준다
    lastbp = heap_listp;
    if (extend_heap(CHUNKSIZE / WSIZE) == NULL) // comment) 더 이상 힙을 확장할 수 없을 때
        return -1;
    return 0;
}
void *mm_malloc(size_t size) {
    ...
    size_t extendsize;
    char *bp;
    // comment) 이 조건은 없어도 되는 조건이지만, 있을 때 이해하기 더 좋은 것 같습니다 
    if (size == 0) {
        return 0;
    }
// 최소 16바이트 크기의 블록 구성
...
// 추가적인 8바이트는 헤더아 풋터 오버헤드를 위해
    if (size <= DSIZE)
        asize = 2 * DSIZE;
        // comment) 할당할 데이터 블락 크기가 DSIZE보다 작을 경우에도 최소 블락 크기 유지시킨다 헤더 + 데이터 + 풋터 포함 16바이트로 넣어줌
    else // 8바이트를 넘는 요청은 오버헤드 바이트 내에 더해주고 인접 8의 배수로 반올림.
        asize = DSIZE * ((size + (DSIZE) + (DSIZE - 1)) / DSIZE);
    if (( bp = find_fit(asize)) != NULL) { // comment) 말록 가능한 공간을 찾았을 때
        place(bp, asize);
        return bp;
    }
...
static void *coalesce(void *bp) { // 인접 가용 블록 합치기
    ...
    // comment: Case 1 조건 없이도 리턴을 해줘도 되지만 오히려 점수가 떨어지네요 지금처럼 유지하는게 좋을 것 같습니다. 
...
static void *extend_heap(size_t words) {
    ...
    /*
        comment)
        malloc에서 요청할 때 8배수로 들어오기 때문에 2워드의 배수로 반올림을 하지 않아도 될 것 같습니다. 
        -1 과 비교 시 -1을 변경해줘도 됩니다.
